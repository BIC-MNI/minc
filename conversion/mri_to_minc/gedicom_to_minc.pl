# Subroutines for GE mri machines, version 6.x dicom

# Routine to initialize tape drive
sub gedicom_initialize_tape_drive {
   &cleanup_and_die("gedicom_to_minc does not support tape reading.\n",1);
}
...
sub gedicom_read_headers {
    ...
    # Check arguements
    if (scalar(@_) != 2) {
        &cleanup_and_die("Argument error in gedicom_read_headers",1);
    }
...
sub gedicom_read_file_info {
    ...
    local(%header);
    undef(%header);
    if (&gedicom_read_headers($filename, *header)) {
        return 1;
    }
...
sub gedicom_get_image_cmd {
    if (scalar(@_) != 2) {
        &cleanup_and_die("Argument error in gedicom_get_image_cmd",1);
    }
...
*initialize_tape_drive = *gedicom_initialize_tape_drive;
*read_file_info = *gedicom_read_file_info;
*get_image_cmd = *gedicom_get_image_cmd;
