...
\desc{Sets the size of the cache block in each dimension.  When a voxel is
accessed in a
cached volume, the corresponding block containing the voxel is read from
or written to the disk as necessary.  This function sets the size of
the block and therefore controls the tradeoff between fast access of
adjacent voxels (using large block sizes) and fast access of arbitrarily
distributed voxels (using small block sizes).  If an application
intends to perform slice-by-slice operations, then a block size
corresponding to one slice may be selected.  If this function is not
called, the default value is a block size of 8 voxels per dimension,
or the value specified by the environment variable
\name{VOLUME\_CACHE\_BLOCK\_SIZE}.  Note that this function may be called
after volume creation, but before the first voxel access.  This allows
a volume to be created via \name{start\_volume\_input}, and the file
sizes and other information to be examined, before setting the block
size to an appropriate value.  After the first voxel access, the block
size for a volume cannot be changed.}
...
\desc{Sets the maximum number of bytes allowed in the cache for each
...
the application to output the volume.  If the application tries to
output this volume to a file of the same name as that set by this
function, the request to output will be ignored, because the volume
