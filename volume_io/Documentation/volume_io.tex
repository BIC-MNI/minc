...
vector does not include the translational component of the transform.}
...
\desc{Multiplies the two matrices \name{t1} and \name{t2}, storing the
...
+ 1) by \name{n\_dimensions}, which is created by the following
function:}
...
\desc{These two functions output the general transform to a file, in the
appropriate format.  The comments line is an arbitrary string which is
...
default extension of \name{.xfm}, outputs the transform, then closes
