...
\name{value}.  This is more efficient than \name{GET\_VOXEL} because the
macro does not look up the number of dimensions in the volume.}
...
dimensional, then the final two arguments are ignored.  This macro just
performs the two macros \name{GET\_VOXEL} and
\name{CONVERT\_VOXEL\_TO\_VALUE}.}
...
\name{value}.  This just performs one of the \name{GET\_VOXEL\_?D}
macros, then a \name{CONVERT\_VOXEL\_TO\_VALUE}.
...
character inserted at the beginning of each line.  The first form of the
function assumes the file is already open
